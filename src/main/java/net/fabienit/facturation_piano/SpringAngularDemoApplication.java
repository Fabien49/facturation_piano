...
public class SpringAngularDemoApplication {
    ...
    CommandLineRunner commandLineRunner(StudentRepository studentRepository, PaymentRepository paymentRepository, BillRepository billRepository) {
        return args -> {
            ...
            billRepository.save(Bill.builder().billStatus(BillStatus.SENT).paymentStatus(PaymentStatus.CREATED).sentDate(LocalDate.now()).amount(68).paymentType(PaymentType.CASH).build());
            billRepository.save(Bill.builder().billStatus(BillStatus.RELAUNCHED).relaunchedDate(LocalDate.now()).paymentStatus(PaymentStatus.CREATED).sentDate(LocalDate.now()).amount(135).paymentType(PaymentType.TRANSFER).build());
            billRepository.save(Bill.builder().billStatus(BillStatus.SET).paymentStatus(PaymentStatus.VALIDATED).sentDate(LocalDate.now()).amount(254).paymentType(PaymentType.CHECK).build());
